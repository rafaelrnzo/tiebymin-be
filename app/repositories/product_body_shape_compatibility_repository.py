...
class ProductBodyShapeCompatibilityRepository(ABC):
    ...
    @abstractmethod
    def update(self, compatibility_id: uuid.UUID, compatibility_data: ProductBodyShapeCompatibilityCreate) -> Optional[ProductBodyShapeCompatibility]:
        pass
...
class SupabaseProductBodyShapeCompatibilityRepository(ProductBodyShapeCompatibilityRepository):
    TABLE_NAME = "product_body_shape_compatibility"
    ...
    def get_all(self) -> List[ProductBodyShapeCompatibility]:
        ...
        return [ProductBodyShapeCompatibility(**item) for item in response.data] if response.data else []

    def get_by_id(self, compatibility_id: uuid.UUID) -> Optional[ProductBodyShapeCompatibility]:
        response = supabase.table(self.TABLE_NAME).select("*").eq("id", str(compatibility_id)).execute()
        return ProductBodyShapeCompatibility(**response.data[0]) if response.data else None
    ...
    def create(self, compatibility_data: ProductBodyShapeCompatibilityCreate) -> ProductBodyShapeCompatibility:
        ...
        if not response.data:
            raise HTTPException(status_code=500, detail="Failed to create product body shape compatibility.")
        return ProductBodyShapeCompatibility(**response.data[0])
