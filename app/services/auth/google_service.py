...
def login_google(request: Request, response: Response, return_url: bool = False):
    ...
    try:
        redirect_uri = settings.google_redirect(request)  # ✅ pakai settings
...
def callback_google(request: Request, code: str, state: str, user_repo: UserRepository):
    ...
    try:
        redirect_uri = settings.google_redirect(request)  # ✅ pakai settings
        print(f"[Google OAuth] Callback using redirect URI: {redirect_uri}")
        ...
        if token_resp.status_code != 200:
            print(f"[Google OAuth] Token request failed: {token_resp.text}")
            raise HTTPException(
                status_code=400,
        ...
        if not user:
            try:
                ...
                print(f"[Google OAuth] Failed to create user: {str(e)}")
                raise HTTPException(
                    status_code=500,
