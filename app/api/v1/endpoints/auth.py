...
from app.schemas.user import User as UserSchema, UserRegistration, UserLogin
...
def register_user(user_data: UserRegistration, user_repo: UserRepository = Depends(get_user_repository)):
    existing_user = user_repo.get_by_email(user_data.email)
    if existing_user:
        raise HTTPException(status_code=400, detail="Email already registered")
    hashed_password = get_password_hash(user_data.password)
    new_user = user_repo.create(UserRegistration(**user_data.dict(), password=hashed_password))
    return new_user
