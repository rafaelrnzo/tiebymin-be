...
# ^ NOTE: ganti "pydantic[email]" -> "email-validator" (lebih aman untuk Pydantic v2)

# Copy project (kode + alembic.ini + folder alembic/)
COPY . .

# Pastikan folder ada (biar .gitkeep tetap bekerja jika kamu pakai)
RUN mkdir -p alembic/versions

# Copy entrypoint & pastikan LF + executable
